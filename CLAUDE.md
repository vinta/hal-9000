...
- All skill descriptions must start with `Use when`, `Use before`, or `Use after` (may have a `(project)` prefix if it's a project-level skill), except skills with `disable-model-invocation: true`, whose description is a human-facing summary.
