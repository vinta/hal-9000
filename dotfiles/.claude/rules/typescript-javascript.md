...
- An action is the bare verb, the gerund is the noun or modifier: `spaceText()`, `spacingMode`. A predicate about whether to act takes the verb (`shouldAutoSpace`); a predicate about the concept's state keeps the noun (`hasProperSpacing`). Feature names stay as their ADR spells them (`applyAiSpacing`), and persisted keys (storage, i18n) are not renamed without a migration
