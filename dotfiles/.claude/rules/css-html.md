...
- Don't use accent or link colors on non-interactive elements (inline code, badges, pills, tags). Reserve accent colors for clickable items. Use a neutral or muted color for decorative/informational elements so users don't mistake them for links.
- Always include `prefers-reduced-motion: reduce` — disable animations and transitions for users who request it.
