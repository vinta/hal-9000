...
Fetch the [Codex memory guide](https://learn.chatgpt.com/docs/customization/memories). Use the memory directory and update contract supplied in the active session. If no location is supplied, inspect `$CODEX_HOME/memories/`, with `~/.codex` as the default Codex home. If memory is unavailable, report that rather than creating a replacement store.
...
For promotions, fetch the [AGENTS.md guide](https://learn.chatgpt.com/docs/agent-configuration/agents-md). Put universal instructions in the Codex home's `AGENTS.md`, project-wide instructions in the project's `AGENTS.md`, and directory-specific instructions in the nearest applicable nested `AGENTS.md`. Keep project instructions out of global guidance.
...
These are task choices. Handle any execution permission or sandbox escalation separately through the environment's approval mechanism.
...
If no authorized update mechanism is available, deliver the concrete change list and state that it has not been applied. User-selected verdicts do not override restrictions on generated files.
