...
3. **Audit topic files.** Give every topic file exactly one verdict, with the evidence beside it. Done when no file lacks one.
   ...
   - **rewrite**: right fact, wrong form: the index hook or `description` disagrees with the body; a `[[link]]` names a filename or a slug no memory carries, since links target the `name:` field; a relative date; two files carrying one fact, which merge into one; the human named as anything but "the user" or the model addressed as anything but "you", the voice the system prompt uses, outside words quoted as user speech.
